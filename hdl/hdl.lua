...
g_Adder = nil;
g_IsInsideSwitch = false;
g_IsInsideCase = false;

HDLF_CONST_COMPARE_EXPAND = 1 << 0;
HDLF_SWITCH_TRISTATE_BUS  = 1 << 1;
g_Flags = 0;
...
function make_index_node(operand, index)
	...
	--	if(index <= operandWidth) then
	--		return make_index_node(operand.operands[1], index);
	--	elseif(index == operandWidth + 1) then
	--		local carry = operand.carry;
	--		assert(carry ~= nil, "Trying to read the carry bit from an adder result but the adder didn't generate a carry bit");
	--		return carry;
	--	end
	...
	--	assert(index > 0 and index <= operandWidth, string.format("Index should be in the range [1, %d], was %d", operandWidth, index));
...
function make_compare_node(op, left, right)
	...
	if((g_Flags & HDLF_CONST_COMPARE_EXPAND) == HDLF_CONST_COMPARE_EXPAND) then
...
function make_register_node(size, clk, rst_n, set_n, next)
	...
	--assert(not next or (isNode(next) and next.size.width == size.width), "Register input 'next' should either be left unconnected or connected to a " .. size.width .. "-bit signal");
...
function switch(node)
	...
	return function (cases)
        ...
        for r, cases in pairs(regs) do
			...
			if((g_Flags & HDLF_SWITCH_TRISTATE_BUS) == HDLF_SWITCH_TRISTATE_BUS) then
				local nodes = {};
				local comps = {};
...
function case(val)
    assert(g_IsInsideSwitch, "case() can only be called inside a switch() block");
    g_IsInsideCase = true;
...
function default()
    assert(g_IsInsideSwitch, "default() can only be called inside a switch() block");
    g_IsInsideCase = true;
    return function (assignments)
        g_IsInsideCase = false;
